...
# Use this script to generate QLocale headers from the data generated by first running locale_cldr2xml.py
...
def fixedScriptName(name, dupes):
    name = name.translate(ReplaceList)
    if name[-6:] != "Script":
        name = name + "Script";
    if name in dupes:
        sys.stderr.write("\n\n\nERROR: Script name '%s' is messy" % name)
        sys.exit(1);
    return name

def fixedCountryName(name, dupes):
    if name in dupes:
        return name.translate(ReplaceList) + "Country"
    return name.translate(ReplaceList)

def fixedLanguageName(name, dupes):
    if name in dupes:
        return name.translate(ReplaceList) + "Language"
    return name.translate(ReplaceList)

def findDupes(country_map, language_map):
    country_set  = set([ v[0] for a, v in country_map.items() ])
    language_set = set([ v[0] for a, v in language_map.items() ])
...
def loadLocaleMap(doc, language_map, script_map, country_map, likely_subtags_map):
    ...
    while locale_elt:
        ...
        if language_id != 1: # C
            ...
            if script_id == 0:
                # find default script for a given language and country (https://www.unicode.org/reports/tr35/#Likely_Subtags)
...
def usage():
    print("Usage: locale_xml2locale.py  <path-to-cs-src>")
    sys.exit(1)

GENERATED_BLOCK_START = "// GENERATED PART STARTS HERE\n"
GENERATED_BLOCK_END   = "// GENERATED PART ENDS HERE\n"

ReplaceList = str.maketrans("", "", " -")
...
def main():
    ...
    doc = xml.dom.minidom.parse(locale_xml)
...
" % (str(datetime.date.today()), cldr_version) )
    ...
    locale_keys.sort(key=cmp_to_key(compareLocaleKeys))
    ...
    qlocaleh_temp_file.write("        Bengali = Bangla,\n")
    ...
    qlocaleh_temp_file.write("        Greenlandic = Kalaallisut,\n")
    qlocaleh_temp_file.write("        Kurundi = Rundi,\n")
    qlocaleh_temp_file.write("        Inupiak = Inupiaq,\n")
    qlocaleh_temp_file.write("        Navaho = Navajo,\n")
    qlocaleh_temp_file.write("        RhaetoRomance = Romansh,\n")
    qlocaleh_temp_file.write("        Uighur = Uyghur,\n")
    qlocaleh_temp_file.write("        Uigur = Uyghur,\n")
    qlocaleh_temp_file.write("\n")
    ...
    qlocaleh_temp_file.write("        BengaliScript = BanglaScript,\n")
    qlocaleh_temp_file.write("        OriyaScript = OdiaScript,\n")
    qlocaleh_temp_file.write("        MendeKikakuiScript = MendeScript,\n")
    qlocaleh_temp_file.write("\n")
    ...
    # rename
    qlocaleh_temp_file.write("\n")
    qlocaleh_temp_file.write("        Bonaire = CaribbeanNetherlands,\n")
    qlocaleh_temp_file.write("        CuraSao = Curacao,\n")
    qlocaleh_temp_file.write("        CzechRepublic = Czechia,\n")
    ...
    qlocaleh_temp_file.write("        EastTimor = TimorLeste,\n")
    qlocaleh_temp_file.write("        Macau = Macao,\n")
    qlocaleh_temp_file.write("        Macedonia = NorthMacedonia,\n")
    ...
    qlocaleh_temp_file.write("        Swaziland = Eswatini,\n")
    qlocaleh_temp_file.write("        UnitedStatesMinorOutlyingIslands = UnitedStatesOutlyingIslands,\n")
    qlocaleh_temp_file.write("        VaticanCityState = VaticanCity,\n")
    qlocaleh_temp_file.write("        WallisAndFutunaIslands = WallisAndFutuna,\n")
    qlocaleh_temp_file.write("\n")
