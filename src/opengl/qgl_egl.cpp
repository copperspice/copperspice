      ...
      if (!d->workaroundsCached) {
         ...
         if ((strstr(renderer, "SGX") || strstr(renderer, "MBX"))) {
            ...
            if (egl_version && strstr(egl_version, "1.3")) {
               ...
               // 1.4 version SGX drivers from Nokia have fixed the brokenFBOReadBack problem
               // official drivers from TI have not been fixed, the beagleboard may be broken

               // to avoid performance issues, only enable workaround_brokenFBOReadBack when needed

               // check for a Nokia-specific addon (see MeeGo bug #5616)

               if (! QEgl::hasExtension("EGL_NOK_image_shared")) {
                  // no Nokia extension found, this is probably a standard SGX driver
