...
static bool qt_setSessionProtocol(SSLContextRef context, const QSslConfigurationPrivate &configuration, QTcpSocket *plainSocket)
{
   ...
   } else if (configuration.protocol == QSsl::TlsV1_0_OrLater) {
      err = SSLSetProtocolVersionMin(context, kTLSProtocol1);
   ...
   } else if (configuration.protocol == QSsl::TlsV1_1_OrLater) {
      err = SSLSetProtocolVersionMin(context, kTLSProtocol11);
   ...
   } else if (configuration.protocol == QSsl::TlsV1_2_OrLater) {
      err = SSLSetProtocolVersionMin(context, kTLSProtocol12);
...
{
   // add call to QSysInfo::prettyProductName();
   return "Secure Transport, Mac OS X";
}
...
bool QSslSocketBackendPrivate::setSessionCertificate(QString &errorDescription, QAbstractSocket::SocketError &errorCode)
{
   ...
   if (!localCertificate.isNull()) {
      ...
      if (configuration.privateKey.isNull()) {
         ...
         errorDescription = QString("Can not provide a certificate with no key");
         return false;
      }

      // import certificates and key
      const QString passPhrase(QString::fromLatin1("foobar"));
      QCFType<CFDataRef> pkcs12 = _q_makePkcs12(configuration.localCertificateChain, configuration.privateKey, passPhrase).toCFData();
      QCFType<CFStringRef> password = passPhrase.toCFString();

      const void *keys[] = { kSecImportExportPassphrase };
      const void *values[] = { password };

      QCFType<CFDictionaryRef> options(CFDictionaryCreate(nullptr, keys, values, 1, nullptr, nullptr));
      ...
      if (!CFArrayGetCount(items)) {
         errorCode = QAbstractSocket::SslInvalidUserDataError;
         errorDescription = QString("SecPKCS12Import returned no items");
         return false;
      }
      ...
      if (!identity) {
         ...
         errorCode = QAbstractSocket::SslInvalidUserDataError;
         errorDescription = QString("SecPKCS12Import returned no identity");
         return false;
      }
      ...
      if (!certs) {
         ...
         errorDescription = QString("Failed to allocate certificates array");
         return false;
      }
...
bool QSslSocketBackendPrivate::canIgnoreTrustVerificationFailure() const
{
   ...
   return mode == QSslSocket::SslServerMode && (verifyMode == QSslSocket::QueryPeer
          || verifyMode == QSslSocket::AutoVerifyPeer || verifyMode == QSslSocket::VerifyNone);
}
...
bool QSslSocketBackendPrivate::verifyPeerTrust()
{
   ...
   const bool doVerifyPeer = verifyMode == QSslSocket::VerifyPeer || (verifyMode == QSslSocket::AutoVerifyPeer
   ...
   for (const QSslCertificate &cert : configuration.caCertificates) {
