...
QNetworkAccessHttpBackendFactory::create(QNetworkAccessManager::Operation op,const QNetworkRequest &request) const
{
    // check the operation
    switch (op) {
    ...
    default:
        // no, we can not handle this request
        return 0;
    }
...
void QNetworkAccessHttpBackend::postRequest()
{
    ...
    if (transparentProxy.type() == QNetworkProxy::DefaultProxy &&
        ...
        QMetaObject::invokeMethod(this, "error", isSynchronous() ? Qt::DirectConnection : Qt::QueuedConnection,
                                  ...
                                  Q_ARG(const QString &, tr("No suitable proxy found")));
    ...
    if (!isSynchronous()) {
        ...
        connect(delegate, SIGNAL(downloadData(const QByteArray &)),this, SLOT(replyDownloadData(const QByteArray &)),
                Qt::QueuedConnection);

        connect(delegate, SIGNAL(downloadFinished()),this, SLOT(replyFinished()),
                Qt::QueuedConnection);

        connect(delegate, SIGNAL(downloadMetaData(
                const QList<QPair<QByteArray,QByteArray> > &,int,const QString &,bool,QSharedPointer<char>,qint64)),
                this,  SLOT(replyDownloadMetaData(
                const QList<QPair<QByteArray,QByteArray> > &,int,const QString &,bool,QSharedPointer<char>,qint64)),
                Qt::QueuedConnection);

        connect(delegate, SIGNAL(downloadProgress(qint64,qint64)),
                this, SLOT(replyDownloadProgressSlot(qint64,qint64)),Qt::QueuedConnection);

        connect(delegate, SIGNAL(error(QNetworkReply::NetworkError,const QString &)),
                this, SLOT(httpError(QNetworkReply::NetworkError, const QString &)), Qt::QueuedConnection);

#ifndef QT_NO_OPENSSL
        connect(delegate, SIGNAL(sslConfigurationChanged(const QSslConfiguration &)),
                this, SLOT(replySslConfigurationChanged(const QSslConfiguration &)),Qt::QueuedConnection);
#endif

        // Those need to report back, therefire BlockingQueuedConnection
        connect(delegate, SIGNAL(authenticationRequired(const QHttpNetworkRequest &,QAuthenticator*)),
                this, SLOT(httpAuthenticationRequired(const QHttpNetworkRequest &,QAuthenticator*)), Qt::BlockingQueuedConnection);

#ifndef QT_NO_NETWORKPROXY
        connect (delegate, SIGNAL(proxyAuthenticationRequired(const QNetworkProxy &,QAuthenticator*)),
                 this, SLOT(proxyAuthenticationRequired(const QNetworkProxy &,QAuthenticator*)), Qt::BlockingQueuedConnection);
#endif

#ifndef QT_NO_OPENSSL
        connect(delegate, SIGNAL(sslErrors(const QList<QSslError> &,bool *,QList<QSslError> *)),
                this,  SLOT(replySslErrors(const QList<QSslError> &,bool *,QList<QSslError> *)), Qt::BlockingQueuedConnection);
#endif
        ...
        if (uploadByteDevice) {
            ...
            // needed to make sure it is moved on moveToThread()
            forwardUploadDevice->setParent(delegate); 
            ...
            connect(forwardUploadDevice, SIGNAL(resetData(bool*)), this, SLOT(resetUploadDataSlot(bool*)), Qt::BlockingQueuedConnection); 
        }
...
void QNetworkAccessHttpBackend::replyDownloadData(const QByteArray &d)
{
    ...
    if (pendingSignals > 0) {
        ...
        // and do it in the next call we get (signal comppression)
...
void QNetworkAccessHttpBackend::replyDownloadMetaData
        (const QList<QPair<QByteArray,QByteArray> > &hm,
         int sc, const QString &rp,bool pu, QSharedPointer<char> db, qint64 contentLength)
{
    statusCode = sc;
    reasonPhrase = rp;
...
void QNetworkAccessHttpBackend::httpAuthenticationRequired(const QHttpNetworkRequest &, QAuthenticator *auth)
{
    authenticationRequired(auth);
}

void QNetworkAccessHttpBackend::httpError(QNetworkReply::NetworkError errorCode,const QString &errorString)
{
#if defined(QNETWORKACCESSHTTPBACKEND_DEBUG)
...
bool QNetworkAccessHttpBackend::sendCacheContents(const QNetworkCacheMetaData &metaData)
{
    ...
    QMetaObject::invokeMethod(this, "writeDownstreamData", isSynchronous() ? Qt::DirectConnection : Qt::QueuedConnection, 
            Q_ARG(QIODevice*, contents));
