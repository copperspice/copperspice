...
QNetworkReplyHttpImpl::QNetworkReplyHttpImpl(QNetworkAccessManager *const manager,
            const QNetworkRequest &request, QNetworkAccessManager::Operation &operation, QIODevice *outgoingData)
   : QNetworkReply(*new QNetworkReplyHttpImplPrivate, manager)
{
   ...
   if (outgoingData) {
      ...
      if (!d->outgoingData->isSequential()) {
         ...
         bool bufferingDisallowed = request.attribute(QNetworkRequest::DoNotBufferUploadDataAttribute, false).toBool();
...
QNetworkReplyHttpImplPrivate::QNetworkReplyHttpImplPrivate()
   : QNetworkReplyPrivate(), manager(nullptr), managerPrivate(nullptr), synchronous(false), state(Idle)
   , statusCode(0), uploadByteDevicePosition(false), uploadDeviceChoking(false)
   , outgoingData(nullptr), bytesUploaded(-1), cacheLoadDevice(nullptr), loadingFromCache(false)
   , cacheSaveDevice(nullptr), cacheEnabled(false), resumeOffset(0), preMigrationDownloaded(-1)
...
bool QNetworkReplyHttpImplPrivate::sendCacheContents(const QNetworkCacheMetaData &metaData)
{
   ...
   for ( ; it != end; ++it) {
      if (httpRequest.isFollowRedirects() && ! qstricmp(it->first.toLower().constData(), "location")) {
...
QNetworkCacheMetaData QNetworkReplyHttpImplPrivate::fetchCacheMetaData(const QNetworkCacheMetaData &oldMetaData) const
{
   ...
   for (QByteArray header : newHeaders) {
      ...
      if (it != cacheHeaders.rawHeaders.constEnd()) {
         ...
         if (header == "content-encoding" || header == "content-range" || header == "content-type") {
            continue;
         }
