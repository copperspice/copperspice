...
bool QSpdyProtocolHandler::sendRequest()
{
   ...
   QMultiMap<int, HttpMessagePair>::iterator iter = m_channel->spdyRequestsToSend.begin();

   // requests will be ordered by priority (see QMultiMap doc)
   for (int a = 0; a < requestsToSend; ++a) {
      HttpMessagePair currentPair = *iter;
      ...
      sendSYN_STREAM(currentPair, streamID, 0);

      iter = m_channel->spdyRequestsToSend.erase(iter);
   }
...
void QSpdyProtocolHandler::handleControlFrame(const QByteArray &frameHeaders)
{
   ...
   if (! readNextChunk(length, frameData.data())) {
      // put back the frame headers in the buffer
      m_spdyBuffer.prepend(frameHeaders);
      return; // we could not read the whole frame and need to wait
   ...
   for (qint32 a = 0; a < headerCount; ++a) {
      ...
      if (name == ":status") {
         ...
         if (value.contains('\0')) {
            ...
            for (const QByteArray &ivalue : values) {
               if (value.isEmpty()) {
                  value = ivalue;
               } else {
                  value += binder + ivalue;
