...
class QPrintPropertiesDialog : public QDialog
{
 ...
 public:
   ...
   void addItemToOptions(QOptionTreeItem *parent, QList<const ppd_option_t *> &options,
                         QList<const char *> &markedOptions) const;
#endif
...
class QPPDOptionsEditor : public QStyledItemDelegate
{
 ...
 private:
   GUI_CS_SLOT_1(Private, void cbChanged(int index))
   GUI_CS_SLOT_2(cbChanged)
...
#if !defined(QT_NO_CUPS)
void QPrintPropertiesDialog::addItemToOptions(QOptionTreeItem *parent, QList<const ppd_option_t *> &options,
      QList<const char *> &markedOptions) const
{
...
void QPrintDialogPrivate::init()
{
   ...
   QObject::connect(options.reverse, SIGNAL(toggled(bool)), q, SLOT(_q_chbPrintLastFirstToggled(bool)));
   ...
   if (parent->type == QOptionTreeItem::Root) {
      ...
      if (ppdFile) {
         for (int i = 0; i < ppdFile->num_groups; ++i) {
            QOptionTreeItem *group = new QOptionTreeItem(QOptionTreeItem::Group, i, &ppdFile->groups[i], ppdFile->groups[i].text,
                  parent);
      ...
      if (group) {
         for (int i = 0; i < group->num_subgroups; ++i) {
            QOptionTreeItem *subgroup = new QOptionTreeItem(QOptionTreeItem::Group, i, &group->subgroups[i],
...
void QPPDOptionsModel::parseOptions(QOptionTreeItem *parent)
{
   ...
   for (int i = 0; i < group->num_options; ++i) {
      QOptionTreeItem *opt = new QOptionTreeItem(QOptionTreeItem::Option, i, &group->options[i], group->options[i].text,
            parent);
...
void QPPDOptionsModel::parseChoices(QOptionTreeItem *parent)
{
   ...
   for (int i = 0; i < option->num_choices; ++i) {
      QOptionTreeItem *choice = new QOptionTreeItem(QOptionTreeItem::Choice, i, &option->choices[i], option->choices[i].text,
            parent);
...
{
   if (index.column() == 1 &&
         reinterpret_cast<QOptionTreeItem *>(index.internalPointer())->type == QOptionTreeItem::Option) {
...
{
   d->_q_printerChanged(un_named_arg1);
}
