...
void QPrintDialogPrivate::setupDestination()
{
   ...
   QObject::connect(fileName, SIGNAL(textChanged(const QString &)), q, SLOT(_q_fileNameEditChanged(const QString &)));
...
void QPrintDialogPrivate::setupPaper()
{
   ...
   QObject::connect(orientationCombo, SIGNAL(activated(int)), q, SLOT(_q_orientSelected(int)));
   ...
   QObject::connect(sizeCombo, SIGNAL(activated(int)), q, SLOT(_q_paperSizeSelected(int)));
}
...
void QPrintDialogPrivate::setupOptions()
{
   ...
   QObject::connect(copies, SIGNAL(valueChanged(int)), q, SLOT(_q_setNumCopies(int)));
   ...
   QObject::connect(rangeCombo, SIGNAL(activated(int)), q, SLOT(_q_printRangeSelected(int)));

   // page range
   firstPage = q->findChild<QSpinBox *>("firstPage");
   firstPage->setRange(1, 9999);
   firstPage->setValue(1);
   QObject::connect(firstPage, SIGNAL(valueChanged(int)), q, SLOT(_q_setFirstPage(int)));

   lastPage = q->findChild<QSpinBox *>("lastPage");
   lastPage->setRange(1, 9999);
   lastPage->setValue(1);
   QObject::connect(lastPage, SIGNAL(valueChanged(int)), q, SLOT(_q_setLastPage(int)));

   // print order
   pageOrderCombo = q->findChild<QComboBox *>("pageOrderCombo");
   QObject::connect(pageOrderCombo, SIGNAL(activated(int)), q, SLOT(_q_pageOrderSelected(int)));
}
