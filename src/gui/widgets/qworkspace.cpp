...
bool QMDIControl::event(QEvent *event)
{
   if (event->type() == QEvent::ToolTip) {
      ...
      QStyle::SubControl ctrl = style()->hitTestComplexControl(QStyle::CC_MdiControls, &opt, helpEvent->pos(), this);
...
void QMDIControl::mouseReleaseEvent(QMouseEvent *event)
{
   ...
   initStyleOption(&opt);
   QStyle::SubControl under_mouse = style()->hitTestComplexControl(QStyle::CC_MdiControls, &opt, event->pos(), this);
...
class QWorkspaceTitleBar : public QWidget
{
 ...
 public:
   ...
   GUI_CS_SIGNAL_1(Public, void doActivate())
   GUI_CS_SIGNAL_2(doActivate)

   GUI_CS_SIGNAL_1(Public, void doNormal())
   GUI_CS_SIGNAL_2(doNormal)

   GUI_CS_SIGNAL_1(Public, void doClose())
   GUI_CS_SIGNAL_2(doClose)

   GUI_CS_SIGNAL_1(Public, void doMaximize())
   GUI_CS_SIGNAL_2(doMaximize)

   GUI_CS_SIGNAL_1(Public, void doMinimize())
   GUI_CS_SIGNAL_2(doMinimize)

   GUI_CS_SIGNAL_1(Public, void doShade())
   GUI_CS_SIGNAL_2(doShade)

   GUI_CS_SIGNAL_1(Public, void showOperationMenu())
   GUI_CS_SIGNAL_2(showOperationMenu)

   GUI_CS_SIGNAL_1(Public, void popupOperationMenu(const QPoint &un_named_arg1))
   GUI_CS_SIGNAL_2(popupOperationMenu, un_named_arg1)

   GUI_CS_SIGNAL_1(Public, void doubleClicked())
   GUI_CS_SIGNAL_2(doubleClicked)

   GUI_CS_SLOT_1(Public, void setActive(bool))
...
class QWorkspaceChild : public QWidget
{
 ...
 public:
   ...
   GUI_CS_SLOT_1(Public, void activate())
   GUI_CS_SLOT_2(activate)
   ...
   GUI_CS_SLOT_1(Public, void internalRaise())
   GUI_CS_SLOT_2(internalRaise)

   GUI_CS_SLOT_1(Public, void titleBarDoubleClicked())
   GUI_CS_SLOT_2(titleBarDoubleClicked)
...
QWidget *QWorkspace::addWindow(QWidget *w, Qt::WindowFlags flags)
{
   ...
   connect(child, SIGNAL(popupOperationMenu(const QPoint &)), this, SLOT(_q_popupOperationMenu(const QPoint &)));
   connect(child, SIGNAL(showOperationMenu()), this, SLOT(_q_showOperationMenu()));
...
void QWorkspacePrivate::_q_updateActions()
{
   ...
   actions[QWorkspacePrivate::MaximizeAct]->setEnabled((windowWidget->windowFlags() & Qt::WindowMaximizeButtonHint) &&
         canResize);
   ...
   : QWidget(parent, Qt::FramelessWindowHint | Qt::SubWindow)
{
   ...
   if (window && (flags & (Qt::WindowTitleHint
      ...
      connect(titlebar, SIGNAL(doActivate()),   this,   SLOT(activate()));
      connect(titlebar, SIGNAL(doClose()),      window, SLOT(close()));
      connect(titlebar, SIGNAL(doMinimize()),   this,   SLOT(showMinimized()));
      connect(titlebar, SIGNAL(doNormal()),     this,   SLOT(showNormal()));
      connect(titlebar, SIGNAL(doMaximize()),   this,   SLOT(showMaximized()));

      connect(titlebar, SIGNAL(popupOperationMenu(const QPoint &)), this, SLOT(popupOperationMenu(const QPoint &)));
      connect(titlebar, SIGNAL(showOperationMenu()),  this, SLOT(showOperationMenu()));
      connect(titlebar, SIGNAL(doShade()),            this, SLOT(showShaded()));
      connect(titlebar, SIGNAL(doubleClicked()),      this, SLOT(titleBarDoubleClicked()));
   }
   ...
   if (!hasBeenResized) {
      cs = childWidget->sizeHint().expandedTo(childWidget->minimumSizeHint()).expandedTo(
              childWidget->minimumSize()).boundedTo(childWidget->maximumSize());
   } else {
      cs = childWidget->size();
   }
   ...
   if (titlebar) {
      ...
      p = QPoint(contentsRect().x(), th + contentsRect().y());
      s = QSize(cs.width() + 2 * frameWidth(), cs.height() + 2 * frameWidth() + th);

   } else {
      p = QPoint(contentsRect().x(), contentsRect().y());
      s = QSize(cs.width() + 2 * frameWidth(), cs.height() + 2 * frameWidth());
   }
   ...
   connect(widgetResizeHandler, SIGNAL(activate()), this, SLOT(activate()));
...
QWidget *QWorkspaceChild::iconWidget() const
{
   if (!iconw) {
      ...
      connect(iconw, SIGNAL(doActivate()),   this, SLOT(activate()));
      connect(iconw, SIGNAL(doClose()),      windowWidget(), SLOT(close()));
      connect(iconw, SIGNAL(doNormal()),     this, SLOT(showNormal()));
      connect(iconw, SIGNAL(doMaximize()),   this, SLOT(showMaximized()));

      connect(iconw, SIGNAL(popupOperationMenu(const QPoint &)), this, SLOT(popupOperationMenu(const QPoint &)));
      connect(iconw, SIGNAL(showOperationMenu()), this, SLOT(showOperationMenu()));
      connect(iconw, SIGNAL(doubleClicked()),     this, SLOT(titleBarDoubleClicked()));
   }
   if (windowWidget()) {
...
void QWorkspaceChild::adjustToFullscreen()
{
   ...
   if (!((QWorkspace *)parentWidget())->d_func()->maxmenubar ||
         style()->styleHint(QStyle::SH_Workspace_FillSpaceOnMaximize, 0, this)) {
...
void QWorkspace::_q_normalizeActiveWindow()
{
   Q_D(QWorkspace);
   d->_q_normalizeActiveWindow();
}

void QWorkspace::_q_minimizeActiveWindow()
{
   Q_D(QWorkspace);
   d->_q_minimizeActiveWindow();
}

void QWorkspace::_q_showOperationMenu()
{
   Q_D(QWorkspace);
   d->_q_showOperationMenu();
}

void QWorkspace::_q_popupOperationMenu(const QPoint &un_named_arg1)
{
   Q_D(QWorkspace);
   d->_q_popupOperationMenu(un_named_arg1);
}

void QWorkspace::_q_operationMenuActivated(QAction *un_named_arg1)
{
   Q_D(QWorkspace);
   d->_q_operationMenuActivated(un_named_arg1);
}

void QWorkspace::_q_updateActions()
{
   Q_D(QWorkspace);
   d->_q_updateActions();
}

void QWorkspace::_q_scrollBarChanged()
{
   Q_D(QWorkspace);
   d->_q_scrollBarChanged();
}
