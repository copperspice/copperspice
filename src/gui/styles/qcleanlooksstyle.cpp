				   ...
				   const QWidget *widget) const
{
   ...
   switch (element) {
	 ...
	 if (const QStyleOptionHeader *header = qstyleoption_cast<const QStyleOptionHeader *>(option)) {
	    ...
	    if (!QPixmapCache::find(pixmapName, cache)) {
		  ...
		  if (header->position != QStyleOptionHeader::End) {
		     ...
		     cachePainter.drawLine(pixmapRect.bottomLeft() + QPoint(3, -1), pixmapRect.bottomRight() + QPoint(-3, -1));
		     cachePainter.setPen(QPen(option->palette.light().color()));
		     cachePainter.drawLine(pixmapRect.bottomLeft() + QPoint(3, 0), pixmapRect.bottomRight() + QPoint(-3,
					   0));
		  }
	       } else {
		  cachePainter.setPen(QPen(dark));
		  ...
		  cachePainter.setPen(QPen(shadow));
		  cachePainter.drawLine(pixmapRect.topRight() + QPoint(-1, 3), pixmapRect.bottomRight() + QPoint(-1, -3));
		  cachePainter.setPen(QPen(option->palette.light().color()));
		  cachePainter.drawLine(pixmapRect.topRight() + QPoint(0, 3), pixmapRect.bottomRight() + QPoint(0,
					-3));
	       }
	       cachePainter.end();
	 ...
	 if (const QStyleOptionProgressBar *bar = qstyleoption_cast<const QStyleOptionProgressBar *>(option)) {
	    ...
	    int progressBarWidth = (progress - bar->minimum) * qreal(maxWidth) / qMax(qreal(1.0),
	 ...
	 if (const QStyleOptionTab *tab = qstyleoption_cast<const QStyleOptionTab *>(option)) {
	    ...
	    if (atBeginning || selected ) {
	       painter->setPen(light);
	       painter->drawLine(x1 + 1, y1 + 2 + 1, x1 + 1, y2 - ((onlyOne || atBeginning) && selected &&
				 leftAligned ? 0 : borderThinkness) - (atBeginning && leftCornerWidget ? 1 : 0));
	       painter->drawPoint(x1 + 1, y1 + 1);
	       painter->setPen(dark);
	       painter->drawLine(x1, y1 + 2, x1, y2 - ((onlyOne || atBeginning)  &&
						       leftAligned ? 0 : borderThinkness) - (atBeginning && leftCornerWidget ? 1 : 0));
	    }
	    // Top
	    {
	       ...
	       if (!selected) {
		  painter->drawLine(beg - 2, y1 + 1, end, y1 + 1);
	       }
	    ...
	    if (lastTab || selected || onlyOne || !nextSelected) {
		  ...
		  painter->drawLine(x2 - 1, y1 + 3, x2 - 1, y2 - ((onlyOne || lastTab) && selected &&
