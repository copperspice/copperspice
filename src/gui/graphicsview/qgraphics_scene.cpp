...
void QGraphicsScenePrivate::_q_emitUpdated()
{
   ...
   const QMetaMethod &metaMethod = q->metaObject()->method(q->metaObject()->indexOfSignal("changed(const QList<QRectF> &)"));
...
void QGraphicsScene::addItem(QGraphicsItem *item)
{
   ...
   if (!item->d_ptr->isDeclarativeItem) {
      if (d->unpolishedItems.isEmpty()) {
         QMetaMethod method = metaObject()->method(metaObject()->indexOfSlot("_q_polishItems()"));
...
bool QGraphicsScenePrivate::dispatchHoverEvent(QGraphicsSceneHoverEvent *hoverEvent)
{
   ...
   QGraphicsItem *commonAncestorItem = (item && !hoverItems.isEmpty()) ? item->commonAncestorItem(hoverItems.last()) : nullptr;
   ...
   bool force, bool ignoreOpacity, bool removingItemFromScene, bool updateBoundingRect)
{
   ...
   if (removingItemFromScene) {
      ...
      const QMetaMethod &metaMethod = q_ptr->metaObject()->method(q_ptr->metaObject()->indexOfSignal("changed(const QList<QRectF> &)"));
...
void QGraphicsScenePrivate::gestureEventHandler(QGestureEvent *event)
{
   ...
   for (int i = 0; i < cachedTargetItems.size(); ++i) {
      ...
      if (!ignoredGestures.isEmpty() && !isPanel) {
         ...
         gestureTargetsAtHotSpots(ignoredGestures, Qt::ReceivePartialGestures, &cachedItemGestures, &targetsSet, nullptr, nullptr);
