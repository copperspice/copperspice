...
{
   if (view() && view()->parent()) {
      if (view()->parent()->metaObject()->className() == "QComboBoxPrivateContainer") {
...
void QAccessibleTable::modelChange(QAccessibleTableModelChangeEvent *event)
{
   ...
   switch (event->modelChangeType()) {
      ...
      case QAccessibleTableModelChangeEvent::ColumnsInserted: {
         ...
         ChildCache newCache;
         ChildCache::const_iterator iter = childToId.constBegin();
...
int QAccessibleTableCell::rowExtent() const
{
   return 1;
}
...
#ifndef QT_NO_TABLEVIEW
   if (const QTableView *tableV = qobject_cast<const QTableView *>(view)) {
      header = tableV->verticalHeader();
   }
#endif
