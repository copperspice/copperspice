...
void QColorDialogStaticData::readSettings()
{
    ...
    const QSettings settings(QSettings::UserScope, "QtProject");         // broom - should we change?

    for (int i = 0; i < int(CustomColorCount); ++i) {
        const QVariant v = settings.value("Qt/customColors/" + QString::number(i));
        if (v.isValid())
...
void QColorDialogStaticData::writeSettings() const
{
    ...
    if (!customSet) {
        ...
        for (int i = 0; i < int(CustomColorCount); ++i)
            settings.setValue("Qt/customColors/" + QString::number(i), customRgb[i]);
    }
#endif
}
...
QStringList QPlatformFileDialogHelper::cleanFilterList(const QString &filter)
{
    ...
    QString f = filter;               // broom - verify this code
