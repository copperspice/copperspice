...
- (void)windowDidResize: (NSNotification *)notification
{
   ...
   if (!(qwidget->windowState() & (Qt::WindowMaximized | Qt::WindowFullScreen)) && [window isZoomed]) {
      ...
      QWindowStateChangeEvent e(Qt::WindowStates(widgetData->window_state & ~Qt::WindowMaximized));
      qt_sendSpontaneousEvent(qwidget, &e);
...
-(BOOL)windowShouldClose: (id)windowThatWantsToClose
{
   ...
   // CopperSpice
   QThreadData *threadData = internal_get_ThreadData(qwidget);
   QScopedLoopLevelCounter counter(threadData);
...
- (NSRect)windowWillUseStandardFrame: (NSWindow *)window defaultFrame: (NSRect)defaultFrame
{
   ...
   frameToReturn.size.width = qMin(frameToReturn.size.width,
                              size.width() + (windowFrameRect.size.width - viewFrameRect.size.width));

   frameToReturn.size.height = qMin(frameToReturn.size.height,
