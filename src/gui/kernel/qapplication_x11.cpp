                                 ...
                                 const char *bg = 0, const char *button = 0)
{
   ...
   if (!gtkStyle && (kdeColors || (button || !resBG.isEmpty() || !resFG.isEmpty()))) {// set app colors
      ...
      QPalette pal(fg, btn, btn.lighter(125), btn.darker(130), btn.darker(120), wfg.isValid() ? wfg : fg, Qt::white, base,
                   bg);
   ...
   if (event->type == XKeyPress || event->type == XKeyRelease) {
      ...
      if (keywidget) {
         ...
         if (d->inPopupMode()) { // no focus widget, see if we have a popup
            keywidget = (QETWidget *) (activePopupWidget()->focusWidget() ? activePopupWidget()->focusWidget() :
                                       activePopupWidget());
...
class QSmSocketReceiver : public QObject
{
 ...
 public:
   ...
   GUI_CS_SLOT_1(Public, void socketActivated(int))
...
static void sm_performSaveYourself(QSessionManagerPrivate *smd)
{
   ...
   QVarLengthArray<char, 1024> buf(qMax(sysconf(_SC_GETPW_R_SIZE_MAX), 1024L));
...
QSessionManager::QSessionManager(QApplication *app, QString &id, QString &key)
   : QObject(app), d_ptr(new QSessionManagerPrivate(this, id, key))
{
   Q_D(QSessionManager);
