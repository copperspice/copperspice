...
void QWidget::destroy(bool destroyWindow, bool destroySubWindows)
{
   ...
   if (testAttribute(Qt::WA_WState_Created)) {
      ...
      for (int i = 0; i < children().size(); ++i) { // destroy all widget children
...
void QWidgetPrivate::setParent_sys(QWidget *parent, Qt::WindowFlags f)
{
   ...
   if (q->isVisible() && q->parentWidget() && q->parent() != q->parentWidget()) {
   ...
   QObject *tempObj    = q->parent();
   QWidget *tempWidget = dynamic_cast<QWidget *> (tempObj);

   if (q->isWindow() || (! tempWidget || tempWidget->isVisible()) || explicitlyHidden) {
...
void QWidgetPrivate::setGeometry_sys(int x, int y, int w, int h, bool isMove)
{
   ...
   const bool isTranslucentWindow = !isOpaque && ptrUpdateLayeredWindowIndirect &&
                                    (data.window_flags & Qt::FramelessWindowHint)
      ...
      switch (m) {
         ...
         case PdmPhysicalDpiX:
            if (d->extra && d->extra->customDpiX) {
               ...
               val = static_cast<QWidget *>(parent())->metric(m);
            } else {
