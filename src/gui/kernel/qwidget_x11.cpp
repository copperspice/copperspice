   ...
   if (wasCreated) {
      ...
      for (int i = 0; i < chlist.size(); ++i) { // reparent children
         ...
         if (obj->isWidgetType()) {
            ...
            // CS added 1/19/2014
            QWidgetPrivate *tempWidget = w->d_func();

            if (xinfo.screen() != tempWidget->xinfo.screen()) {

               // NOTE: force setParent() to not shortcut out (because we are setting the
               // parent to the current parent) setParent will add child back to the list
               // of children so we need to make sure the widget will not be added twice.

               CSInternalChildren::set_mParent(w, 0);
               CSInternalChildren::removeOne(q, w);

               w->setParent(q);

            } else if (! w->isWindow()) {
               tempWidget->invalidateBuffer(w->rect());
               ...
               if (w->internalWinId()) {
                  if (w->testAttribute(Qt::WA_NativeWindow)) {
                     ...
                     XReparentWindow(X11->display, w->internalWinId(), nativeParentWidget->internalWinId(), p.x(), p.y());

                  } else {
                     tempWidget->setParent_sys(q, w->data->window_flags);
                  }
               }
...
void QWidgetPrivate::setWSGeometry(bool dontShow, const QRect &)
{
   ...
   if (parentWRect.isValid()) {
      ...
      if (data.wrect.isValid() && QRect(QPoint(), data.crect.size()).contains(data.wrect)) {
         ...
         // move our window, and do not need to move or clip children
...
void QWidgetPrivate::setGeometry_sys(int x, int y, int w, int h, bool isMove)
{
   ...
   if (q->isWindow()) {
      ...
      if (isResize &&
            !q->testAttribute(
               Qt::WA_DontShowOnScreen)) { // set config pending only on resize, see qapplication_x11.cpp, translateConfigEvent()
...
void QWidgetPrivate::scroll_sys(int dx, int dy, const QRect &r)
{
   ...
   if (!valid_rect && ! q->children().isEmpty()) {        // scroll children
      QPoint pd(dx, dy);
      for (int i = 0; i < q->children().size(); ++i) { // move all children
         QObject *object = q->children().at(i);
      ...
      switch (m) {
         ...
         case PdmPhysicalDpiX:
            if (d->extra && d->extra->customDpiX) {
               ...
               val = static_cast<QWidget *>(this->parent())->metric(m);
            } else {
