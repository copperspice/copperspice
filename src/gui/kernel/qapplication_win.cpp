...
class QETWidget : public QWidget                // event translator widget
{
 public:
   ...
   inline void showChildren(bool spontaneous) {
      d_func()->showChildren(spontaneous);
   }
   inline void hideChildren(bool spontaneous) {
      d_func()->hideChildren(spontaneous);
   }
   inline uint testWindowState(uint teststate) {
      return dataPtr()->window_state & teststate;
   }
   inline void setWindowTitle_helper(const QString &title) {
...
void qt_init(QApplicationPrivate *priv, int)
{
   ...
   // Initialize OLE/COM
   //    S_OK means success and S_FALSE means that it has already been initialized
   HRESULT r;
   r = OleInitialize(0);

   if (r != S_OK && r != S_FALSE) {
      qWarning("CopperSpice: Could not initialize OLE (error %x)", (unsigned int)r);
   }
...
const QString qt_reg_winclass(QWidget *w)        // register window class
{
   ...
   // has already been registered by another instance then
...
void QApplication::winFocus(QWidget *widget, bool gotFocus)
{
   ...
   if (gotFocus) {
      ...
      if (QApplicationPrivate::active_window && (QApplicationPrivate::active_window->windowType() == Qt::Dialog)) {
   ...
   */
   ...
   switch (message) {
      ...
      case WM_ENDSESSION: {
         ...
         if (endsession && !qAppPriv->aboutToQuitEmitted) {
            ...
            // simulates emit
            QApplication::staticMetaObject().invokeMethod(qApp, "aboutToQuit" );
   ...
   if (qt_is_translatable_mouse_event(message)) {
      ...
      if (! qt_tabletChokeMouse) {
         ...
         // Sometimes we only get a WM_MOUSEMOVE message and sometimes we get both a WM_MOUSEMOVE and
         // a WM_LBUTTONDOWN/UP, this creates a spurious mouse press/release event, using the PeekMessage
         // will help us fix this.  This leaves us with a question:
         //    effectively kills using the mouse AND the tablet simultaneously, may create wacky input
         ...
         if (is_mouse_move) {
            ...
            if (PeekMessage(&msg1, msg.hwnd, WM_MOUSEFIRST, WM_MOUSELAST, PM_NOREMOVE))
               next_is_button = (msg1.message == WM_LBUTTONUP || msg1.message == WM_LBUTTONDOWN);
         }
         ...
         case WM_SETTINGCHANGE:
            ...
            if (! msg.wParam) {
               ...
               if (msg.lParam != 0)  {
                  ...
                  area = QString::fromStdWString(tmp);
               }
...
/*****************************************************************************
  Modal widgets; We have implemented our own modal widget mechanism to get total control.
  A modal widget without a parent becomes application-modal.
  A modal widget with a parent becomes modal to its parent and grandparents..

  QApplicationPrivate::enterModal()
        Enters modal state
        Arguments: QWidget *widget        A modal widget
...
bool qt_try_modal(QWidget *widget, MSG *msg, int &ret)
{
   ...
   if (type != WM_NCHITTEST) {
      ...
      if ((type >= WM_MOUSEFIRST && type <= WM_MOUSELAST) ||
            ...
            type == WM_MOUSELEAVE || (type >= WM_KEYFIRST && type <= WM_KEYLAST) || type == WM_NCMOUSEMOVE
                                     ...
                                     int numPackets)
{
   ...
   for (i = 0; i < numPackets; i++) {
      ...
      if (btnNew) {
#ifndef QT_NO_TABLETEVENT
         if (currentTabletPointer.currentPointerType == QTabletEvent::Pen ||
               currentTabletPointer.currentPointerType == QTabletEvent::Eraser)
...
bool QETWidget::translateConfigEvent(const MSG &msg)
{
   ...
   if (msg.message == WM_SIZE) {                // resize event
      ...
      if (isWindow()) {                        // update title/icon text
         ...
         // Capture SIZE_MINIMIZED without preceding WM_SYSCOMMAND (like Windows+M)
