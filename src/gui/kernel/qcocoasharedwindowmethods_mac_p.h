...
-(void)registerDragTypes
{
   ...
   if (currentCustomDragTypes == 0 || *currentCustomDragTypes != customTypes) {
      ...
      *currentCustomDragTypes = customTypes;
      ...
      const NSString *mimeTypeGeneric = @"com.copperspice.MimeTypeName";
...
- (NSDragOperation)draggingUpdated: (id <NSDraggingInfo>)sender
{
   ...
   if (qt_mac_mouse_inside_answer_rect(localPoint)
         && QT_PREPEND_NAMESPACE(qt_mac_dnd_answer_rec.lastOperation) == nsActions) {
      NSDragOperation operation = QT_PREPEND_NAMESPACE(qt_mac_mapDropActions)(QT_PREPEND_NAMESPACE(
                                     qt_mac_dnd_answer_rec.lastAction));
