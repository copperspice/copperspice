...
void QFontDatabasePrivate::addQPF2File(const QByteArray &file)
{
   ...
   if (data && data != (const uchar *)MAP_FAILED) {
      if (QFontEngineQPF::verifyHeader(data, dataSize)) {
         ...
         QByteArray writingSystemBits = QFontEngineQPF::extractHeaderField(data,
                                        QFontEngineQPF::Tag_WritingSystems).toByteArray();
...
bool QFontDatabasePrivate::loadFromCache(const QString &fontPath)
{
   ...
   if (originalFontPath != fontPath) {
      ...
      qFatal("QFontDatabase::loadFromCache: Font path doesn't match. Found %s in database, expected %s",
             qPrintable(originalFontPath), qPrintable(fontPath));
   }
      ...
      do {
         ...
         if (buf[0] != '#') {
            ...
            if (file[0] && QFile::exists(filename)) {
               db->addFont(QString::fromUtf8(name), /*foundry*/"", weight, italic, size / 10,
                           QFile::encodeName(filename), /*fileIndex*/ 0, smooth);
            }
         }
      } while (!feof(fontdef));
      fclose(fontdef);
