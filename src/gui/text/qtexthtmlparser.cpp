...
static_assert(MAX_ENTITY == sizeof entities / sizeof *entities, "Size of QTextHtmlEntity is incorrect");
...
int QTextHtmlParser::margin(int i, int mar) const
{
   ...
   if (mar == MarginLeft || mar == MarginRight) {
      while (i) {
         node = &at(i);
...
void QTextHtmlParser::parseTag()
{
   ...
   while (pos < len && txt.at(pos) != QChar('>')) {
      if (txt.at(pos) == QChar('/')) {
         tagClosed = true;
      }
...
void QTextHtmlParser::parseCloseTag()
{
   ...
   int p = last();
   if (p > 0 && at(p - 1).tag == tag && at(p - 1).mayNotHaveChildren()) {
      p--;
   }
   ...
   if ((at(p).wsm == QTextHtmlParserNode::WhiteSpacePre || at(p).wsm == QTextHtmlParserNode::WhiteSpacePreWrap)
         && at(p).isBlock()) {

      if (at(last()).text.endsWith('\n')) {
...
void QTextHtmlParser::parseExclamationTag()
{
   ...
   if (hasPrefix(QChar('-'), 1) && hasPrefix(QLatin1Char('-'), 2)) {
      pos += 3;

      // consume comments
      int end = txt.indexOf("-->", pos);
      pos = (end >= 0 ? end + 3 : len);
...
QString QTextHtmlParser::parseWord()
{
   ...
   if (hasPrefix('\"')) {
      ...
      while (pos < len) {
         ...
         if (c == '>' || (c == '/' && hasPrefix('>', 1)) || c == '<' || c == '=' || c.isSpace()) {
            --pos;
            break;
         }
   ...
   switch (id) {
      case Html_a:
         ...
         for (int i = 0; i < attributes.count(); i += 2) {
            ...
            if (key.compare("href", Qt::CaseInsensitive) == 0 && !attributes.at(i + 1).isEmpty()) {
               hasHref = true;
            }
         }
   ...
   for (int i = 0; i < declarations.count(); ++i) {
      ...
      switch (decl.d->propertyId) {
         case QCss::BorderColor:
            borderBrush = QBrush(decl.colorValue());
            break;
         ...
         case QCss::Color:
            charFormat.setForeground(decl.colorValue());
            break;
         ...
         case QCss::QtParagraphType:
            if (decl.d->values.first().variant.toString().compare("empty", Qt::CaseInsensitive) == 0) {
               isEmptyParagraph = true;
            }
            break;

         case QCss::QtTableType:
            if (decl.d->values.first().variant.toString().compare("frame", Qt::CaseInsensitive) == 0) {
               isTextFrame = true;

            } else if (decl.d->values.first().variant.toString().compare("root", Qt::CaseInsensitive) == 0) {
               isTextFrame = true;
               isRootFrame = true;
            }
            break;
         ...
         case QCss::PageBreakBefore:
            switch (identifier) {
               case QCss::Value_Always:
                  blockFormat.setPageBreakPolicy(blockFormat.pageBreakPolicy() | QTextFormat::PageBreak_AlwaysBefore);
                  break;
         ...
         case QCss::PageBreakAfter:
            switch (identifier) {
               case QCss::Value_Always:
                  blockFormat.setPageBreakPolicy(blockFormat.pageBreakPolicy() | QTextFormat::PageBreak_AlwaysAfter);
                  break;
               case QCss::Value_Auto:
                  blockFormat.setPageBreakPolicy(blockFormat.pageBreakPolicy() & ~QTextFormat::PageBreak_AlwaysAfter);
                  break;
...
QStringList QTextHtmlParser::parseAttributes()
{
   ...
   while (pos < len) {
      ...
      if (hasPrefix(QChar('>')) || hasPrefix(QChar('/'))) {
         break;
      }
      ...
      switch (node->id) {
         case Html_font:
            ...
            if (key == "size" && value.size()) {
               ...
               if (value.at(0) != QChar('+') && value.at(0) != '-') {
                  n -= 3;
               }
...
void QTextHtmlParser::resolveStyleSheetImports(const QCss::StyleSheet &sheet)
{
   for (int i = 0; i < sheet.importRules.count(); ++i) {
      ...
      if (rule.media.isEmpty() || rule.media.contains("screen", Qt::CaseInsensitive)) {
   ...
   switch (node.id) {
      ...
      case Html_u: {
         ...
         if (node.id == Html_a) {
            for (int i = 0; i < node.attributes.count(); i += 2) {
               ...
               if (key.compare("href", Qt::CaseInsensitive) == 0 && ! node.attributes.at(i + 1).isEmpty()) {
                  needsUnderline     = true;
                  decl.d->property   = "color";
         ...
         if (needsUnderline) {
            ...
            decl.d->property   = "text-decoration";
      ...
      case Html_small:
         if (node.id != Html_th) {
            ...
            switch (node.id) {
               case Html_h1:
                  val.variant = QVariant(QCss::Value_XXLarge);
                  break;
               case Html_h2:
                  val.variant = QVariant(QCss::Value_XLarge);
                  break;
               case Html_h3:
               case Html_big:
                  val.variant = QVariant(QCss::Value_Large);
                  break;
               case Html_h4:
                  val.variant = QVariant(QCss::Value_Medium);
                  break;
               case Html_h5:
               case Html_small:
                  val.variant = QVariant(QCss::Value_Small);
                  break;
               default:
                  break;
            }
      ...
      case Html_sup:
         decl = QCss::Declaration();
         decl.d->property   = "vertical-align";
      ...
      case Html_nobr:
         ...
         switch (node.id) {
            case Html_br:
               val.variant = QVariant(QCss::Value_PreWrap);
               break;

            case Html_nobr:
               val.variant = QVariant(QCss::Value_NoWrap);
               break;

            case Html_pre:
               val.variant = QVariant(QCss::Value_Pre);
               break;
