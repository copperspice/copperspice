...
private:
    static constexpr inline quint64 alphaMask() { return Q_UINT64_C(0xffff) << AlphaShift; }

    static constexpr inline quint8 div_257_floor(uint x) { return quint8((x - (x >> 8)) >> 8); }
    static constexpr inline quint8 div_257(quint16 x) { return div_257_floor(x + 128U); }
    static constexpr inline quint16 div_65535(uint x) { return quint16((x + (x>>16) + 0x8000U) >> 16); }
