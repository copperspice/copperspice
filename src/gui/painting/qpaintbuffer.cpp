...
void QPaintBufferEngine::drawTextItem(const QPointF &pos, const QTextItem &ti)
{
   ...
   if (m_stream_raw_text_items) {
      QPaintBufferCommand *cmd = buffer->addCommand(QPaintBufferPrivate::Cmd_DrawTextItem,
                                 QVariant::fromValue<void *>(new QTextItemIntCopy(ti)));
...
{
   connect(QPaintBufferSignalProxy::instance(), SIGNAL(aboutToDestroy(const QPaintBufferPrivate *)), this,
           SLOT(remove(const QPaintBufferPrivate *)));
}
   ...
   for (int i = 0; i < variants.size(); ++i) {
      ...
      if (v.canConvert<QPaintBufferCacheEntry>()) {
         ...
         if (entry.flags.type == QPaintBufferCacheEntryV2::ImageKey) {
            ...
            qWarning() << "operator<<(QDataStream &stream, QPaintBuffer &buffer): unrecognized cache entry type:" <<
                       entry.flags.type;
         }
      }
   }
