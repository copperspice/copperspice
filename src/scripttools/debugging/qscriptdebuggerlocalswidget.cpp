...
{
    CS_OBJECT(QScriptDebuggerLocalsItemDelegate)
...
void QScriptDebuggerLocalsWidget::setLocalsModel(QScriptDebuggerLocalsModel *model)
{
    ...
    if (model) {
        QObject::connect(model, SIGNAL(scopeObjectAvailable(const QModelIndex &)), this, SLOT(_q_expandIndex(const QModelIndex &)));
    }
