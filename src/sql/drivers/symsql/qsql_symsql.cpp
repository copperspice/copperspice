...
*/
bool QSymSQLDriver::open(const QString &db, const QString &, const QString &, const QString &, int,
   ...
   if (res == KErrNotFound) {
      ...
      if (policyIndex != -1) {
         ...
         if (policyIndex == -1) {
            ...
            res = KErrArgument;
            setLastError(gMakeErrorOpen(tr("POLICY_DB_DEFAULT must be defined before any other POLICY definitions can be used"),
...
static QSqlIndex qGetTableInfo(QSqlQuery &q, QString &tableName, bool onlyPIndex = false)
{
   ...
   q.exec(QLatin1String("PRAGMA ") + dbName + QLatin1String("table_info (") + _q_escapeIdentifier(
             table) + QLatin1String(")"));
