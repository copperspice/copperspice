...
bool regex_search(BidiIterator first, BidiIterator last, match_results<BidiIterator, Allocator> &m,
...
bool regex_search(BidiIterator first, BidiIterator last, const basic_regex<charT, traits> &e, match_flag_type flags = match_default)
{
   ...
   using match_alloc_type =  typename match_results<BidiIterator>::allocator_type;

   match_results<BidiIterator> m;
   cs_regex_detail_ns::perl_matcher<BidiIterator, match_alloc_type, traits> matcher(first, last, m, e, flags | regex_constants::match_any, first);
