...
{
   // Caller must lock mutex first
   if (!QURL_HASFLAG(stateFlags, Parsed)) {
      parse();
   } else {
      ensureEncodedParts();
   }
   ...
   if ((options & QUrl::RemoveAuthority) != QUrl::RemoveAuthority && (!auth.isNull() || doFileScheme ||
         !savedHost.isEmpty())) {
...
bool QUrl::isValid() const
{
   ...
   if (!QURL_HASFLAG(d->stateFlags, QUrlPrivate::Validated)) {
      d->validate();
   }
...
bool QUrl::operator <(const QUrl &url) const
{
   ...
   if (!d) {
      return url.d ? QByteArray() < url.d->normalized() : false;
   }
   if (!QURL_HASFLAG(d->stateFlags, QUrlPrivate::Parsed)) {
      d->parse();
   }
   if (!url.d) {
      return d->normalized() < QByteArray();
   }
   if (!QURL_HASFLAG(url.d->stateFlags, QUrlPrivate::Parsed)) {
      url.d->parse();
   }
...
{
   if (!d) {
      return QLatin1String(QT_TRANSLATE_NOOP(QUrl,
                                             "Invalid URL \"\": "));   // XXX not a good message, but the one an empty URL produces
   }
   QMutexLocker lock(&d->mutex);
