      ...
      const QXmlName name)
{
   ...
   if (!s) {
      ...
      if (StandardLocalNames::QName == localName) {
         ...
         s = addFunction(StandardLocalNames::doc, 1, 1, CommonSequenceTypes::ZeroOrOneDocumentNode,
                         Expression::DisableElimination);
         s->appendArgument(argument(np, "uri"), CommonSequenceTypes::ZeroOrOneString);
      } else if (StandardLocalNames::doc_available == localName) {
         s = addFunction(StandardLocalNames::doc_available, 1, 1, CommonSequenceTypes::ExactlyOneBoolean,
                         Expression::DisableElimination);
         s->appendArgument(argument(np, "uri"), CommonSequenceTypes::ZeroOrOneString);
         ...
         s = addFunction(StandardLocalNames::static_base_uri, 0, 0, CommonSequenceTypes::ExactlyOneAnyURI,
                         Expression::EmptynessFollowsChild);
