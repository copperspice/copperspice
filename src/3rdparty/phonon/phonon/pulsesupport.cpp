...
static void logMessage(const QString &message, int priority, QObject *obj)
{
    if (debugLevel() > 0) {
        ...
        if (priority <= debugLevel()) {
            qDebug() << QString::fromLatin1("PulseSupport(%1): %2").formatArg(priority).formatArg(output);
        }
    }
}
...
static void createGenericDevices()
{
    ...
    s_captureDevices.insert(index, AudioDevice(QLatin1String("source:default"),
            QObject::tr("PulseAudio Sound Server"), QLatin1String("audio-backend-pulseaudio"), 0));
...
static void ext_device_manager_read_cb(pa_context *c, const pa_ext_device_manager_info *info, int eol, void *userdata) {
    ...
    if (eol < 0) {
        logMessage(QString("Failed to initialize device manager extension: %1").formatArg(QString::fromLatin1(pa_strerror(pa_context_errno(c)))));
    ...
    if (eol) {
        // We are done reading the data, so order it by priority and copy it into the
        ...
        for (int i = Phonon::NoCategory; i <= Phonon::LastCategory; ++i) {
            ...
            if (s_outputDevicePriorities.contains(cat)) {
                logMessage(QString("  Phonon Category %1").formatArg(cat));
                int count = 0;

                for (int j : s_outputDevicePriorities[cat]) {
                    QHash<QByteArray, QVariant> &props = s_outputDevices[j].properties;
                    logMessage(QString("    %1. %2 (Available: %3)")
                        .formatArg(++count).formatArg(props["name"].toString()).formatArg(props["available"].toBool()));
                }
            }
        }
    ...
    if (name.startsWith("sink:")) {
        ...
        // indicates a bug in pulseaudio.
        return;
    }

    // Add the new device itself.
    new_devices->insert(name, AudioDevice(name, QString::fromLatin1(info->description), QString::fromLatin1(info->icon), info->index));
    ...
    for (uint32_t i = 0; i < info->n_role_priorities; ++i) {
        ...
        if (s_roleCategoryMap.contains(QString::fromLatin1(role_prio->role))) {
            Phonon::Category cat = s_roleCategoryMap[QString::fromLatin1(role_prio->role)];
...
static void ext_device_manager_subscribe_cb(pa_context *c, void *) {
    ...
    if (! (o = pa_ext_device_manager_read(c, ext_device_manager_read_cb, u))) {
        logMessage("pa_ext_device_manager_read() failed.");
        delete u;
        return;
    }
...
void sink_input_cb(pa_context *c, const pa_sink_input_info *i, int eol, void *userdata) {
    ...
    if (eol < 0) {
        ...
        logMessage("Sink input callback failure");
        return;
    }
    ...
    if ((t = pa_proplist_gets(i->proplist, "phonon.streamid"))) {
        ...
        logMessage(QString("Found PulseAudio stream index %1 for Phonon Output Stream %2")
                  .formatArg(i->index).formatArg(QString::fromLatin1(t)));

        s_outputStreamIndexMap[QString::fromLatin1(t)] = i->index;
        ...
        if (PA_INVALID_INDEX != i->sink) {
            ...
            if (found) {
                ...
                logMessage("Letting the rest of phonon know about this");
                s_instance->emitUsingDevice(QString::fromLatin1(t), device);
            }
        }
    }
}
...
void source_output_cb(pa_context *c, const pa_source_output_info *i, int eol, void *userdata) {
    ...
    if (eol < 0) {
        ...
        logMessage("Source output callback failure");
        return;
    }
    ...
    if ((t = pa_proplist_gets(i->proplist, "phonon.streamid"))) {
        logMessage(QString("Found PulseAudio stream index %1 for Phonon Capture Stream %2")
                  .formatArg(i->index).formatArg(QString::fromLatin1(t)));

        s_captureStreamIndexMap[QString::fromLatin1(t)] = i->index;
            ...
            if ((t & PA_SUBSCRIPTION_EVENT_TYPE_MASK) == PA_SUBSCRIPTION_EVENT_REMOVE) {
                ...
                if (! phononid.isEmpty()) {
                    ...
                    if (s_outputStreamIndexMap.contains(phononid)) {
                        logMessage(QString("Phonon Output Stream %1 is gone at the PA end. Marking it as invalid in our cache as we may reuse it.")
                              .formatArg(phononid));

                        s_outputStreamIndexMap[phononid] = PA_INVALID_INDEX;
                    } else {
                        logMessage(QString::fromLatin1("Removing Phonon Output Stream %1 (it is gone)").formatArg(phononid));
            ...
            if ((t & PA_SUBSCRIPTION_EVENT_TYPE_MASK) == PA_SUBSCRIPTION_EVENT_REMOVE) {
                ...
                if (!phononid.isEmpty()) {
                    if (s_captureStreamIndexMap.contains(phononid)) {
                        logMessage(QString::fromLatin1("Phonon Capture Stream %1 is gone at the PA end. Marking it as invalid in the cache.")
                              .formatArg(phononid));

                        s_captureStreamIndexMap[phononid] = PA_INVALID_INDEX;

                    } else {
                        logMessage(QString::fromLatin1("Removing Phonon Capture Stream %1 (it is gone)").formatArg(phononid));
                ...
                if (!(o = pa_context_get_source_output_info(c, index, source_output_cb, NULL))) {
                    logMessage("pa_context_get_sink_input_info() failed");
                    return;
                }
                pa_operation_unref(o);
            }
...
static QString statename(pa_context_state_t state)
{
    ...
    return QString::fromLatin1("Unknown state: %0").formatArg(state);
}
...
static void context_state_callback(pa_context *c, void *)
{
    ...
    logMessage(QString::fromLatin1("context_state_callback %1").formatArg(statename(pa_context_get_state(c))));
 ...
 : QObject(), mEnabled(false)
{
    ...
    if (QAbstractEventDispatcher::instance()->metaObject()->className() != "QGuiEventDispatcherGlib") {
    ...
    if (pa_context_connect(p_test_context, NULL, static_cast<pa_context_flags_t>(0), NULL) < 0) {
        logMessage(QString::fromLatin1("PulseAudio support disabled: %1")
               .formatArg(QString::fromLatin1(pa_strerror(pa_context_errno(p_test_context)))));
...
static void setDevicePriority(Category category, QStringList list)
{
    ...
    logMessage(QString::fromLatin1("Reindexing %1: %2").formatArg(role).formatArg(list.join(QLatin1String(", "))));
...
void PulseSupport::setStreamPropList(Category category, QString streamUuid)
{
    ...
    logMessage(QString::fromLatin1("Setting role to %1 for streamindex %2").formatArg(role).formatArg(streamUuid));
...
bool PulseSupport::setOutputDevice(QString streamUuid, int device) {
    ...
    if (!s_outputDevices.contains(device)) {
        logMessage(QString::fromLatin1("Attempting to set Output Device for invalid device id %1.").formatArg(device));
        return false;
    }
    const QVariant var = s_outputDevices[device].properties["name"];
    logMessage(QString::fromLatin1("Attempting to set Output Device to '%1' for Output Stream %2")
            .formatArg(var.toString()).formatArg(streamUuid));
    ...
    if (s_outputStreamIndexMap.contains(streamUuid) && s_outputStreamIndexMap[streamUuid] != PA_INVALID_INDEX) {
        ...
        logMessage(QString::fromLatin1("Moving Pulse Sink Input %1 to '%2' (Pulse Sink %3)")
               .formatArg(pulse_stream_index).formatArg(var.toString()).formatArg(pulse_device_index));
...
bool PulseSupport::setCaptureDevice(QString streamUuid, int device) {
    ...
    if (!s_captureDevices.contains(device)) {
        logMessage(QString::fromLatin1("Attempting to set Capture Device for invalid device id %1.").formatArg(device));
        return false;
    }
    const QVariant var = s_captureDevices[device].properties["name"];

    logMessage(QString::fromLatin1("Attempting to set Capture Device to '%1' for Capture Stream %2")
            .formatArg(var.toString()).formatArg(streamUuid));

    // Attempt to look up the pulse stream index.
    if (s_captureStreamIndexMap.contains(streamUuid) && s_captureStreamIndexMap[streamUuid] == PA_INVALID_INDEX) {
        logMessage("... Found in map. Moving now");
        ...
        logMessage(QString::fromLatin1("Moving Pulse Source Output %1 to '%2' (Pulse Sink %3)")
               .formatArg(pulse_stream_index).formatArg(var.toString()).formatArg(pulse_device_index));
...
void PulseSupport::clearStreamCache(QString streamUuid) {
    ...
    return;
#else
    logMessage(QString::fromLatin1("Clearing stream cache for stream %1").formatArg(streamUuid));
