...
double CachedResource::currentAge() const
{
    ...
    double apparentAge = std::isfinite(dateValue) ? max(0., m_responseTimestamp - dateValue) : 0;
    double ageValue = m_response.age();
    double correctedReceivedAge = std::isfinite(ageValue) ? max(apparentAge, ageValue) : apparentAge;
