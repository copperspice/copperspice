...
static bool attachTypeLibrary(const QString &applicationName, int resource, const QByteArray &data,
                              QString *errorMessage)
{
   HANDLE hExe = BeginUpdateResource((const wchar_t *)applicationName.utf16(), false);
   if (hExe == 0) {
      if (errorMessage) {
         *errorMessage = QString::fromLatin1("Failed to attach type library to binary %1 - could not open file.").arg(
                            applicationName);
      }
      return false;
   }
   if (!UpdateResource(hExe, L"TYPELIB", MAKEINTRESOURCE(resource), 0, (void *)data.data(), data.count())) {
      ...
      if (errorMessage) {
         *errorMessage = QString::fromLatin1("Failed to attach type library to binary %1 - could not update file.").arg(
                            applicationName);
      }
      return false;
   }
   ...
   if (!EndUpdateResource(hExe, false)) {
      if (errorMessage) {
         *errorMessage = QString::fromLatin1("Failed to attach type library to binary %1 - could not write file.").arg(
                            applicationName);
      }
      return false;
   }
...
static HRESULT dumpIdl(const QString &input, const QString &idlfile, const QString &version)
{
   ...
   if (input.endsWith(QLatin1String(".exe"))) {
      if (runWithQtInEnvironment(quotePath(input) + QLatin1String(" -dumpidl ") + idlfile + QLatin1String(" -version ") +
                                 version)) {
         res = S_OK;
      }
   } else {
