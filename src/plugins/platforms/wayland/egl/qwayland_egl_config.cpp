...
QEglConfigChooser::QEglConfigChooser(EGLDisplay display)
   : m_ignore(false), m_confAttrRed(0), m_confAttrGreen(0), m_confAttrBlue(0), m_confAttrAlpha(0),
    m_display(display), m_surfaceType(EGL_WINDOW_BIT)
{
}
...
EGLConfig QEglConfigChooser::chooseConfig()
{
   ...
   if (! cfg) {
      qWarning("Unable to find EGLConfig");
   }
...
bool QEglConfigChooser::filterConfig(EGLConfig config) const
{
   ...
   if (m_confAttrAlpha != 0) {
      eglGetConfigAttrib(display(), config, EGL_ALPHA_SIZE, &alpha);
   }

   return red == m_confAttrRed && green == m_confAttrGreen && blue == m_confAttrBlue && alpha == m_confAttrAlpha;
}

EGLConfig q_configFromGLFormat(EGLDisplay display, const QSurfaceFormat &format,
      bool highestPixelFormat, int surfaceType)
{
...
QSurfaceFormat q_glFormatFromConfig(EGLDisplay display, const EGLConfig config, const QSurfaceFormat &referenceFormat)
{
   ...
   format.setStereo(false);         // EGL does not support stereo buffers
...
{
#ifndef Q_OS_LINUX
   (void) framebufferDevice;
#endif

   constexpr const int defaultPhysicalDpi = 100;
   static QSizeF size;
   ...
   if (size.isEmpty()) {
      ...
      if (framebufferDevice != -1) {
         if (ioctl(framebufferDevice, FBIOGET_VSCREENINFO, &vinfo) == -1) {
            qWarning("q_physicalScreenSizeFromFb() Unable to query screen information");
         } else {
            w = vinfo.width;
            h = vinfo.height;
...
QSize q_screenSizeFromFb(int framebufferDevice)
{
   ...
   constexpr const int defaultWidth  = 800;
   constexpr const int defaultHeight = 600;
   ...
   if (size.isEmpty()) {
      ...
      if (framebufferDevice != -1) {
         if (ioctl(framebufferDevice, FBIOGET_VSCREENINFO, &vinfo) == -1) {
            qWarning("q_screenSizeFromFb() Unable to read screen information");
         } else {
            xres = vinfo.xres;
            yres = vinfo.yres;
         }
...
int q_screenDepthFromFb(int framebufferDevice)
{
   ...
   constexpr const int defaultDepth = 32;
   ...
   if (depth == 0) {
      ...
      if (framebufferDevice != -1) {
         if (ioctl(framebufferDevice, FBIOGET_VSCREENINFO, &vinfo) == -1) {
            qWarning("CsWayland_Egl: Unable to query screen infoformation");
         } else {
