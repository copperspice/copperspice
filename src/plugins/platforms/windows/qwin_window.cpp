...
static bool applyBlurBehindWindow(HWND hwnd)
{
   ...
   static PtrDwmEnableBlurBehindWindow dwmBlurBehind = nullptr;
   static PtrDwmIsCompositionEnabled dwmIsCompositionEnabled = nullptr;
   ...
   DwmBlurBehind blurBehind = {0, 0, nullptr, 0};
...
static void setWindowOpacity(HWND hwnd, Qt::WindowFlags flags, bool hasAlpha, bool openGL, qreal level)
{
   if (QWindowsWindow::setWindowLayered(hwnd, flags, hasAlpha, level)) {
      if (hasAlpha && !openGL && (flags & Qt::FramelessWindowHint)) {
         ...
         QWindowsContext::user32dll.updateLayeredWindow(hwnd, nullptr, nullptr, nullptr, nullptr, nullptr, 0, &blend, ULW_ALPHA);
      } else {
         QWindowsContext::user32dll.setLayeredWindowAttributes(hwnd, 0, (int)(level * 255), LWA_ALPHA);
      }

   } else if (IsWindowVisible(hwnd)) {
      // Repaint when switching from layered.
...
struct WindowCreationData {
   ...
   WindowCreationData()
      : parentHandle(nullptr), type(Qt::Widget), style(0), exStyle(0),
...
QWindowsWindowData WindowCreationData::create(const QWindow *w, const WindowData &data, QString title) const
{
   ...
   const HINSTANCE appinst = (HINSTANCE)GetModuleHandle(nullptr);
...
{
   // NCCALCSIZE_PARAMS structure if wParam == 1
...
QWindowsWindow::QWindowsWindow(QWindow *aWindow, const QWindowsWindowData &data)
   : QPlatformWindow(aWindow), m_data(data), m_flags(WithinCreate), m_hdc(nullptr),
     m_windowState(Qt::WindowNoState), m_opacity(1.0), m_cursor(new CursorHandle), m_dropTarget(nullptr),
     m_savedStyle(0), m_format(aWindow->requestedFormat()), m_iconSmall(nullptr),
     m_iconBig(nullptr), m_surface(nullptr)
{
...
void QWindowsWindow::updateTransientParent() const
{
   if (window()->type() == Qt::Popup) {
      return;   // QTBUG-34503, apopup stays on top, no parent, see also WindowCreationData::fromWindow().
   }
   ...
   if (w->isTopLevel()) {
      ...
      if (state & Qt::WindowMinimized) {
         ...
         if (state & Qt::WindowMaximized) {
            ...
            if (flags & Qt::WindowTitleHint && ! (flags & (Qt::WindowMinMaxButtonsHint | Qt::FramelessWindowHint))) {
               fakedMaximize = 1;
...
void QWindowsWindow::setParent_sys(const QPlatformWindow *parent)
{
   ...
   // nullptr handle means desktop window, which also has its proper handle -> disambiguate
...
bool QWindowsWindow::handleWmPaint(HWND hwnd, UINT message, WPARAM, LPARAM)
{
   ...
   if (!GetUpdateRect(m_data.hwnd, nullptr, FALSE)) {
      return false;
   }
   ...
   if (testFlag(OpenGLSurface) && QOpenGLStaticContext::opengl32.moduleIsNotOpengl32()
   ...
   // Consider making it dependent on !DwmIsCompositionEnabled().
   ...
   // we still need to send isExposed=true, for compatibility. Our tests depend on it.
...
QMargins QWindowsWindow::frameMargins() const
{
   ...
   if (testFlag(FrameDirty)) {
      ...
      m_data.frame = m_data.flags & Qt::FramelessWindowHint
         ? QMargins(0, 0, 0, 0) : QWindowsGeometryHint::frame(style(), exStyle());
...
#ifndef QT_NO_CURSOR
   if (m_cursor->isNull()) {
      // Recurse up to parent with non-null cursor. Set default for toplevel.
...
void QWindowsWindow::setCustomMargins(const QMargins &newCustomMargins)
{
   if (newCustomMargins != m_data.customMargins) {
      ...
      SetWindowPos(m_data.hwnd, nullptr, newFrame.x(), newFrame.y(), newFrame.width(), newFrame.height(),
...
void QWindowsWindow::registerTouchWindow(QWindowsWindowFunctions::TouchWindowTouchTypes touchTypes)
{
   if ((QWindowsContext::instance()->systemInfo() & QWindowsContext::SI_SupportsTouch)
      ...
      // Return if it is not a touch window or the flags are already set by a hook such as HCBT_CREATEWND
      if (ret || touchFlags != 0) {
         return;
      }
