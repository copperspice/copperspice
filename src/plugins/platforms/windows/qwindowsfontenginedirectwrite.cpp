...
glyph_t QWindowsFontEngineDirectWrite::glyphIndex(char32_t ch) const
{
   UINT16 glyphIndex;

   HRESULT hr = m_directWriteFontFace->GetGlyphIndicesW(&ch, 1, &glyphIndex);
...
bool QWindowsFontEngineDirectWrite::stringToCMap(QStringView strView, QGlyphLayout *glyphs,
   int *nglyphs, QFontEngine::ShaperFlags flags) const
{
   ...
   QVector<UINT32> codePoints;
   QVector<UINT16> glyphIndices(len);

   while (auto ch : strView) {
      codePoints.append(ch.unicode());
   }

   HRESULT hr = m_directWriteFontFace->GetGlyphIndicesW(codePoints.constData(), len, glyphIndices.data());
