...
bool QWindowsWinTab32DLL::init()
{
   ...
   QSystemLibrary library(QString("wintab32"));
   if (!library.load()) {
      return false;
   }
...
QWindowsTabletSupport *QWindowsTabletSupport::create()
{
   ...
   const HWND window = QWindowsContext::instance()->createDummyWindow(QString("TabletDummyWindow"),
         L"TabletDummyWindow",
   ...
   if (!context) {
      qDebug() << __FUNCTION__ << "Unable to open tablet.";
...
QString QWindowsTabletSupport::description() const
{
   ...
   QString result = QString("%1 specification: v%2.%3 implementation: v%4.%5 options: 0x%6")
      .formatArg(QString::fromStdWString(std::wstring(winTabId.data())))
      .formatArg(specificationVersion >> 8).formatArg(specificationVersion & 0xFF)
      .formatArg(implementationVersion >> 8).formatArg(implementationVersion & 0xFF)
      .formatArg(opts, 0, 16);

   if (opts & CXO_MESSAGES) {
      result += QString(" CXO_MESSAGES");
   }

   if (opts & CXO_CSRMESSAGES) {
      result += QString(" CXO_CSRMESSAGES");
   }
...
bool QWindowsTabletSupport::translateTabletProximityEvent(WPARAM /* wParam */, LPARAM lParam)
{
   ...
   if (!LOWORD(lParam)) {
      qDebug() << "leave proximity for device #" << m_currentDevice;
   ...
   qDebug() << "enter proximity for device #"
