...
HRESULT STDMETHODCALLTYPE AccessibleApplication::QueryInterface(REFIID id, LPVOID *iface)
{
    *iface = 0;
    if (id == IID_IUnknown) {
        qDebug() << "AccessibleApplication::QI(): IID_IUnknown";
        *iface = static_cast<IUnknown *>(this);
    } else if (id == IID_IAccessibleApplication) {
        qDebug() << "AccessibleApplication::QI(): IID_IAccessibleApplication";
...
HRESULT STDMETHODCALLTYPE QWindowsIA2Accessible::get_uniqueID(long *outUniqueID)
{
    ...
    qDebug() << "uniqueID: " << showbase << hex << id;
...
HRESULT STDMETHODCALLTYPE QWindowsIA2Accessible::get_indexInParent(long *indexInParent)
{
    ...
    *indexInParent = par ? par->indexOfChild(accessible) : -1;
    ...
    if (*indexInParent < 0) {
        qWarning() << "index in parent invalid:" << accessible << "parent:" << par;
        return S_FALSE;
    }
    return S_OK;
}
...
HRESULT STDMETHODCALLTYPE QWindowsIA2Accessible::QueryService(REFGUID guidService, REFIID riid, void **iface)
{
    ...
    *iface = 0;
    qDebug() << "QWindowsIA2Accessible::QS(): " << IIDToString(riid);
...
// static_assert(IA2_ROLE_CANVAS == QAccessible::Canvas); // ### Qt 6: make them the same

static_assert(IA2_ROLE_COLOR_CHOOSER == static_cast<IA2Role>(QAccessible::ColorChooser), "");
static_assert(IA2_ROLE_FOOTER == static_cast<IA2Role>(QAccessible::Footer) "");
static_assert(IA2_ROLE_FORM == static_cast<IA2Role>(QAccessible::Form) "");
static_assert(IA2_ROLE_HEADING == static_cast<IA2Role>(QAccessible::Heading) "");
static_assert(IA2_ROLE_NOTE == static_cast<IA2Role>(QAccessible::Note) "");
static_assert(IA2_ROLE_COMPLEMENTARY_CONTENT == static_cast<IA2Role>(QAccessible::ComplementaryContent) "");
