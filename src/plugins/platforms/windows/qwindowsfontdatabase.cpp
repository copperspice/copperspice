...
static inline DWriteCreateFactoryType resolveDWriteCreateFactory()
{
   ...
   QSystemLibrary library(QString("dwrite"));
...
QStringList QWindowsFontDatabase::addApplicationFont(const QByteArray &fontData, const QString &fileName)
{
   ...
   if (! fontData.isEmpty()) {
      ...
      if (AddFontResourceExW(fileName.toStdWString().data(), FR_PRIVATE, 0) == 0) {
         return QStringList();
      }
...
void QWindowsFontDatabase::removeApplicationFonts()
{
   for (const WinApplicationFont &font : m_applicationFonts) {
      if (font.handle) {
         ...
         RemoveFontResourceExW(font.fileName.toStdWString().data(), FR_PRIVATE, 0);
      }
   }
   ...
   const QSharedPointer<QWindowsFontEngineData> &data)
{
   ...
   if (useDirectWrite && initDirectWrite(data.data())) {
      const QString fam = QString::fromStdWString(std::wstring(lf.lfFaceName));
...
QFont QWindowsFontDatabase::systemDefaultFont()
{
   ...
   if (systemFont.family() == QLatin1String("MS Shell Dlg")) {
      systemFont.setFamily(QString("MS Shell Dlg 2"));
   }
