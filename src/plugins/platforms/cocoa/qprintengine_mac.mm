...
int QMacPrintEngine::metric(QPaintDevice::PaintDeviceMetric m) const
{
   ...
   switch (m) {
      ...
      case QPaintDevice::PdmWidthMM:
         val = qRound(d->m_pageLayout.paintRect(QPageSize::Millimeter).width());
         break;

      case QPaintDevice::PdmHeightMM:
         val = qRound(d->m_pageLayout.paintRect(QPageSize::Millimeter).height());
         break;
      ...
      case PPK_Duplex: {
         ...
         switch (mode) {
            ...
            case QPrint::DuplexAuto:
               PMSetDuplex(d->settings(),
                     d->m_pageLayout.orientation() == QPageLayout::Landscape ? kPMDuplexTumble: kPMDuplexNoTumble);
               break;
...
QVariant QMacPrintEngine::property(PrintEnginePropertyKey key) const
{
   ...
   switch (key) {
      ...
      case PPK_PageMargins: {
         QList<QVariant> list;
         QMarginsF margins = d->m_pageLayout.margins(QPageSize::Point);
