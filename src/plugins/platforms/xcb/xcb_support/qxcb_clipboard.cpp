...
class QXcbClipboardMime : public QXcbMime
{
 ...
 protected:
   ...
   QVariant retrieveData_sys(const QString &fmt, QVariant::Type requestedType) const override {
      ...
      return mimeConvertToFormat(m_clipboard->connection(), fmtatom, m_clipboard->getDataInFormat(modeAtom, fmtatom), fmt, requestedType,
            encoding);
   }
 private:
...
QXcbClipboard::~QXcbClipboard()
{
   ...
   if (m_timestamp[QClipboard::Clipboard] != XCB_CURRENT_TIME ||
      ...
      xcb_get_selection_owner_reply_t *reply  = xcb_get_selection_owner_reply(xcb_connection(), cookie, nullptr);
...
void QXcbClipboard::handleSelectionRequest(xcb_selection_request_event_t *req)
{
   ...
   if (req->target == multipleAtom) {
      ...
      if (req->property == XCB_NONE || !clipboardReadProperty(req->requestor, req->property, false, &multi_data,
            nullptr, &multi_type, &multi_format) || multi_format != 32) {
...
bool QXcbClipboard::clipboardReadProperty(xcb_window_t win, xcb_atom_t property, bool deleteProperty, QByteArray *buffer, int *size,
   ...
   *format = reply->format;
   ...
   if (ok && newSize) {
      ...
      while (bytes_left) {
         ...
         reply = xcb_get_property_reply(xcb_connection(), cookie, nullptr);
