...
#ifndef QXCB_CONNECTION_H
#define QXCB_CONNECTION_H
...
class QXcbWindowEventListener
{
 public:
   ...
   virtual bool handleGenericEvent(xcb_generic_event_t *, long *) {
      return false;
   }
...
class QXcbConnection : public QObject
{
 ...
 public:
   ...
   QXcbConnection *connection() const {
      return const_cast<QXcbConnection *>(this);
   }
   ...
   QXcbVirtualDesktop *primaryVirtualDesktop() const {
      return m_virtualDesktops.value(m_primaryScreenNumber);
   }

   QXcbScreen *primaryScreen() const;

   inline xcb_atom_t atom(QXcbAtom::Atom atom) const {
   ...
   using PeekFunc = bool (*)(QXcbConnection *, xcb_generic_event_t *);

   void addPeekFunc(PeekFunc f);

   inline xcb_timestamp_t time() const {
      return m_time;
   }

   inline void setTime(xcb_timestamp_t t) {
      if (t > m_time) {
         m_time = t;
      }
   }

   inline xcb_timestamp_t netWmUserTime() const {
      return m_netWmUserTime;
   }

   inline void setNetWmUserTime(xcb_timestamp_t t) {
      if (t > m_netWmUserTime) {
         m_netWmUserTime = t;
      }
   }

   bool hasXFixes() const {
      return xfixes_first_event > 0;
   }
   bool hasXShape() const {
      return has_shape_extension;
   }
   bool hasXRandr() const {
      return has_randr_extension;
   }
   bool hasInputShape() const {
      return has_input_shape;
   }
   bool hasXKB() const {
      return has_xkb;
   }

   bool supportsThreadedRendering() const {
      return m_reader->isRunning();
   }
   bool threadedEventHandling() const {
      return m_reader->isRunning();
   }
   ...
   void setButton(Qt::MouseButton button, bool down) {
      if (down) {
         m_buttons |= button;
      } else {
         m_buttons &= ~button;
   ...
   QXcbEventReader *eventReader() const {
      return m_reader;
   }
 ...
 private:
   ...
   QXcbScreen *createScreen(QXcbVirtualDesktop *virtualDesktop, const xcb_randr_output_change_t &outputChange,
...
// passed in event. If the passed in event is less than 32 bytes, memcpy() reaches into unrelated memory.
