...
set CMAKE_PREFIX_PATH=..\..\build\release\install\cmake\CopperSpice

mkdir x64
mkdir x64\release
pushd x64\release
::cmake -DCMAKE_INSTALL_PREFIX=install -DCMAKE_BUILD_TYPE=Release -Wno-dev -G "Visual Studio 17 2022" %SRC%
cmake -DCMAKE_INSTALL_PREFIX=install -DCMAKE_BUILD_TYPE=Release -Wno-dev -G Ninja %SRC%
popd

mkdir x64\debug
pushd x64\debug
::cmake -DCMAKE_INSTALL_PREFIX=install -DCMAKE_BUILD_TYPE=Debug -Wno-dev -G "Visual Studio 17 2022" %SRC%
cmake -DCMAKE_INSTALL_PREFIX=install -DCMAKE_BUILD_TYPE=Debug -Wno-dev -G Ninja %SRC%
popd
