...
TEST_CASE("QTextBlock traits", "[qtextblock]")
{
   REQUIRE(std::is_copy_constructible_v<QTextBlock> == true);
   REQUIRE(std::is_move_constructible_v<QTextBlock> == true);

   REQUIRE(std::is_copy_assignable_v<QTextBlock> == true);
   REQUIRE(std::is_move_assignable_v<QTextBlock> == true);

   REQUIRE(std::has_virtual_destructor_v<QTextBlock> == false);
}
